...
## Purpose
When multiple clubs or stakeholders want different things, someone has to analyze the requests, find common patterns, identify conflicts, and propose a solution that maximizes coverage. The synthesis itself requires judgment (Jesse's job), but the analysis, comparison, and per-stakeholder communication can be automated. This is a high-cognitive-load Director task.
