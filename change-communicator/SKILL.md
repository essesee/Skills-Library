...
For each audience, **invoke the `cyrano-write` skill** as the drafting step. Pass the audience name (the recipient profile to target — group, channel, or stakeholder), the change details (what/why/impact/action/timeline), and the change type so `cyrano-write` can adjust tone. The standard structure and tone calibration below guide the intent you pass to `cyrano-write`; do not hand-draft inline using templates alone.

The output structure should follow `references/change-templates.md`:
