---
name: skill-evolver
description: "Use when a skill needs improvement after real usage, or when explicitly asked to analyze a skill. Automatic gate check after skill completions; deep 8-lens analysis on request."
---
...
**If learning period (usage_count < 5):** Before proceeding, run the validation checklist and record results in optimizer-observations.md under a `## Validation Status` section:
- [ ] Routed correctly (this was the right skill for the task)
- [ ] Dependencies available (MCP tools, reference files, project structure)
- [ ] Workflow completed end-to-end (no abandoned steps)
- [ ] Output quality acceptable (user didn't discard or heavily rework)

If 3+ uses pass all checks, add `validated: true` to the observations file. If any check fails twice, flag for explicit review in evolution-log.md.
...
| # | Lens | Key Checks |
|---|------|------------|
| 1 | **Convention Compliance** | Structure vs. `skill-conventions.md`: batch limits, consolidation rules, Context Rules, section order, frontmatter, writing style, description format |
...
| 9 | **Progressive Disclosure** | SKILL.md line count (flag >500); section word counts (flag >500 words); reference depth (flag >1 level); reference files without TOC (flag >100 lines) |
